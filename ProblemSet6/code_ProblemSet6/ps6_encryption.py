...
def buildCoder(shift):
    ...
    for i in range(len(lettersUpper)):
        cipher[lettersUpper[i]] = lettersUpper[(i+shift)%26]
    for i in range(len(lettersLower)):
        cipher[lettersLower[i]] = lettersLower[(i+shift)%26]
    return cipher
...
def findBestShift(wordList, text):
    ...
    realWords, bestShift, textCopy = 0, 0, ''
    for i in range(26):
        textCopy = applyShift(text, i)
        validWords = 0
        for j in textCopy.split():    
